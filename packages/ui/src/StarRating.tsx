...
export const starRatingStarVariants = cva("relative inline-block", {
  variants: {
    size: {
      sm: "h-4 w-4",
      md: "h-5 w-5",
...
export const starRatingGlyphVariants = cva("text-[var(--accent)]", {
  variants: {
    size: {
      sm: "h-4 w-4",
      md: "h-5 w-5",
...
export const starRatingClearVariants = cva(
  "text-xs text-[var(--muted)] hover:text-[var(--ink)]",
  {
    variants: {
      visible: {
        true: "",
...
export type StarRatingVariantProps = VariantProps<typeof starRatingStarVariants>;
export type StarRatingSize = NonNullable<StarRatingVariantProps["size"]>;

type StarRatingProps = StarRatingVariantProps & {
  value: number | null;
    ...
    <div className="flex flex-nowrap items-center gap-2">
      ...
      >
        {stars.map((star) => {
          ...
          return (
            <span key={star} className={starRatingStarVariants({ size })}>
              ...
              >
                <StarGlyph
                  className={starRatingGlyphVariants({ size, preview })}
                />
              </span>
              <button
                type="button"
      ...
      <span className="flex w-[6.5rem] shrink-0 items-center justify-end gap-2">
        ...
        <button
          ...
          disabled={value == null}
          className={starRatingClearVariants({ visible: value != null })}
        >
          Clear
        </button>
      </span>
