...
@Controller("watch/trakt")
export class TraktController {
  constructor(private readonly trakt: TraktService) {}

  @Get("status")
  @UseGuards(SessionUserGuard)
  status(@CurrentWatchUserId() userId: string) {
    return this.trakt.getConnection(userId);
  }
  ...
  authorize(@Res() res: Response, @CurrentWatchUserId() userId: string) {
    ...
    const url = this.trakt.authUrl(state);
  ...
  async callback(
    ...
    const result = await this.trakt.exchangeCode(code, verified.userId);
    const web =
      process.env.WEB_ORIGIN ||
      process.env.NEXT_PUBLIC_WEB_URL ||
      "http://localhost:3000";
    return res.redirect(
      `${web.replace(/\/$/, "")}/watch/settings?trakt=connected`,
    );
  }
