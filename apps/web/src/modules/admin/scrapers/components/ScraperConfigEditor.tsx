...
export const FieldRuleRows = ({ fields, onChange }: Props) => {
  const updateRow = (index: number, patch: Partial<ScraperFieldRule>) => {
...
export const ScraperConfigEditor = ({
  draft,
  onChange,
  hideMeta,
}: EditorProps) => {
  const patchConfig = (patch: Partial<ScraperDefinition>) => {
    onChange({ ...draft, config: { ...draft.config, ...patch } });
  };

  const patchLimits = (patch: Partial<ScraperDefinition["limits"]>) => {
    patchConfig({ limits: { ...draft.config.limits, ...patch } });
  };

  const patchPagination = (pagination: ScraperPagination) => {
